...
public class StateAgent : AIAgent
{
    ...
    public StateAgent enemy { get; private set; }
    private void Start()
    {
        // add states to state machine
        stateMachine.AddState(nameof(AIAttackState), new AIAttackState(this));
        stateMachine.AddState(nameof(AIControlState), new AIControlState(this));
    ...
    private void Update()
    {
        ...
        if (enemySeen)
        {
            enemy = enemies[0].TryGetComponent(out StateAgent stateAgent) ? stateAgent : null;
            enemyDistance.value = Vector3.Distance(transform.position, enemy.transform.position);
            enemyHealth.value = enemy.health;
        }

        // from any state health -> health
